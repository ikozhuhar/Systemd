...
1. Description - имя и основные функции модуля.
2. Documentation - указывает местоположение документации.
3. Requires - перечисление модулей, от которых зависит это модуль (служба или устройство).
4. Wants - директива похожа на Requires, но менее строгая, потому что не требует обязательного запуска зависимостей для работы модуля.
5. Before - модули, перечисленные в этой директиве, не будут запущенны до тех пор, пока текущий модуль не будет отмечен как запущенный.
6. After - модули, перечисленные в этой директиве, будут запущенны до запустка текущего модуля.
7. Conflicts - перечень модулей, которые нельзя запускать одновременно с текущим.
|
